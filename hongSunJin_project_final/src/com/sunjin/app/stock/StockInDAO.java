...
public class StockInDAO extends DAO {
	...
	public void insert(StockInfo info) {
		try {
			connect();
			String sql = "INSERT INTO stock_in (isn, amount) VALUES (?, ?)";
	...
	public boolean selectedInfo(int isn) {
		...
		try {
			...
			String sql = "SELECT COUNT(*) AS COUNT FROM stock_in WHERE isn = " + isn;
	...
	public int SelectedAmount(int isn) {
		...
		try {
			...
			String sql = "SELECT NVL(SUM(amount),0) AS SUM FROM stock_in WHERE isn = " + isn;
	  ...
	  // 총 재고 리스트 조회
	   public List<StockInfo> selectAll() {
	      ...
	      try {
	         ...
	         String sql = "SELECT i.stock_date, i.isn, p.brand, p.product_name, i.amount FROM product p JOIN stock_in i"
	               + " ON p.isn = i.isn" + " ORDER BY i.stock_date";
	...
	public List<StockInfo> selectAll(Date stockDate) {
		...
		try {
			...
			String sql = "SELECT i.stock_date, i.isn, p.product_name, i.amount" + " FROM product p JOIN stock_in i"
					+ " ON p.isn = i.isn" + " WHERE i.stock_date = ?" + " ORDER BY i.stock_date DESC";
