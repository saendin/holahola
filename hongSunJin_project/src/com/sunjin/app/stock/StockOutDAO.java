...
public class StockOutDAO extends DAO {
	...
	public void insert(StockInfo info) {
		try {
			...
			String sql = "INSERT INTO out_stock (isn, stock) VALUES (?, ?)";
	...
	public boolean selectedInfo(int isn) {
		...
		try {
			...
			String sql = "SELECT COUNT(*) AS count FROM out_stock WHERE isn = " + isn;
	...
	public int SelectAmount(int isn) {
		...
		try {
			...
			String sql = "SELECT NVL(SUM(stock), 0) AS SUM FROM out_stock WHERE isn = " + isn;
	...
	public List<StockInfo> selectAll() {
		...
		try {
			...
			String sql = "SELECT o.stock_date, o.isn, p.product_name, o.stock FROM product p JOIN out_stock o"
					+ " ON p.isn = o.isn" + " ORDER BY isn";
	...
	public List<StockInfo> selectAll(Date stockDate) {
		...
		try {
			...
			String sql = "SELECT o.stock_date, o.isn, p.isn, t.stock"
					+ " FROM products p JOIN out_stock o" + " ON p.isn = o.isn"
					+ " WHERE stock_date = ?" + " ORDER BY o.stock_date";
