...
public class productDAO extends DAO {
	...
	private void insert(Product product) {
		try {
			...
			//sql 구성
			String sql = 
					"INSERT INTO products VALUES (product_id, product_name, product_price, product_stock)" //쿼리 통해서 값 넣으면 DEFAULT 구문이 돌지 않으므로 ?가 아닌 값을 직접 입력해줘야함
					+ "VALUES (product_sez.nextval, ? ?)";
//			"VALUES (product_sez.nextval, ? ?) 에 대입될 값 입력
	...
	private void update(Product product) {
		try {
			...
			//sql 구성
			String sql = 
					"UPDATE products SET product_stock = " + product.getProductStock() 
	...
	private void delete(Product productId) {
		try {
			connect();
			
			//sql 구성
			String sql = 
					"DELETE FROM products WHERE product_id = " + productId; //줄 나눠쓸거면 끝 공백 신경 써야함. -> 보기에만 줄이 바뀌는 것. ex) DELETE FROM product 뒤에 꼭 공백 넣어주고 줄 바꿀 것. 아니면 WHERE이랑 붙어버림
	...
	public Product selectOne(String productName) {
		...
		try {
			...
			String sql = "SELECT * FROM products WHERE product_name =" + productName; //primary값이 아니라 중복 값 들어갈 수 있음. 세심하게 조건 주던가 조심해서 등록하던가
			stmt = con.createStatement();
			
			int result = stmt.executeUpdate(sql);
			
			//if문 - 한개의 값 가지고 오기.
			//while에서도 한개만 가지고 오긴 하지만 대신 가장 최신걸로 덮어씀 - 쿼리상에서는 단건조회가 아니고, 자바에서 값을 제한해서 그렇게 보여지는 것 뿐(최대한 자바에서 제어하려고 하지 말것.)
			if(rs.next()) {
				product = new Product();
